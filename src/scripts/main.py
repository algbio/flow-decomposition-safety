...
def maximal_safety_indices(graph, in_flow_decomposition=None, timers=False):
    # Variable timers is not used
    # I disagree with the second TODO comment: the idea of maintaining indices is not to maintain the list of vertices
    '''
        ...
        while True:
            ...
            if f > 0:
                ...
                if not added:
                    ...
                    added = True
                sub = [x for x in sub[1:len(sub)]]  # is not this just sub[1:len(sub)] ?
...
# This method is quadratic in the size of the output, the reason for this is that every time we are concatenating
# strings of increasing size, which in python creates a new object. See for example this
...
if __name__ == '__main__':
    ...
    # Argument mode does not seem to be used, and it seems that the default is not the 'indices' mode
